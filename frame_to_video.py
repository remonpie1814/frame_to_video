...
class CamBuffer:
    ...
    def get_resolution(self):
        return [max(self.capture.get(cv2.CAP_PROP_FRAME_WIDTH), self.min_width), max(self.capture.get(cv2.CAP_PROP_FRAME_HEIGHT), self.min_height)]
    ...
    def queryframe(self):
        try:
            while (not self.isstop):
                ...
                if not self.status:
                    print("cctv 영상을 가져올 수 없습니다...")
                    break
                tmp = cv2.resize(tmp, (max(tmp.shape[1], self.min_width), max(tmp.shape[0], self.min_height)))
...
class FrameToVideo:
    ...
    def run(self, retry=3):
        ...
        try:
            while True:
                if len(self.cam.frames)<1:
                    ...
                    # print("not frame!")
                    time.sleep(0.1)
                    if not_frame_count >= 60 * 30:
                        print(f"not frame for 1 minutes... reset camera...")
                        if retry < 1:
                            break
                        self.cam.reset()
                        retry -= 1
        ...
        except subprocess.TimeoutExpired as tse:
            if not os.path.exists(self.video_url):
                print(tse)
                print(self.video_url, " is not saved ...")
        except Exception as e:
            print(e)
        finally:
            self.process.kill()
